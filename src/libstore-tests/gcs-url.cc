...
TEST(ParsedGCSURL, toHttpsUrlCustomEndpoint)
{
    ...
    EXPECT_EQ(
        p.toHttpsUrl("http", ParsedURL::Authority{.host = "emu.internal", .port = 4443}).to_string(),
        "http://emu.internal:4443/b/k");
    EXPECT_EQ(
        p.toHttpsUrl("https", parseURL("http://server:9000/prefix")).to_string(), "http://server:9000/prefix/b/k");
}
