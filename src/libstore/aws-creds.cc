...
static std::shared_ptr<Aws::Crt::Auth::ICredentialsProvider> createWrappedProvider(
    aws_credentials_provider * rawProvider,
...
public:
    AwsCredentialProviderImpl()
    {
        ...
        tlsContext = std::make_shared<Aws::Crt::Io::TlsContext>(tlsCtxOptions, Aws::Crt::Io::TlsMode::CLIENT, allocator);
    }
...
AwsCredentialProviderImpl::createProviderForProfile(const std::string & profile)
{
    ...
    auto profileProvider = Aws::Crt::Auth::CredentialsProvider::CreateCredentialsProviderProfile(profileConfig, allocator);
    if (profileProvider) {
