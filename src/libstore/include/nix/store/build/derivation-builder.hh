...
/**
 * This type exists to cut down @ref Store to what @ref DerivationBuilder
 * actually needs. This serves two purposes:
 *
 * - Modularity: Now we better understand the requirements different
 *   components impose on one another. Conversely, @ref Store is more
 *   or less the union of many interfaces' requirements, so it obscures
 *   the division of labor to require it everywhere.
 *
 * - FFI: we cannot make a full @ref LocalStore with everything
 *   (including building, which uses this!) from FFI, but we do have a
 *   chance of making something that just has the methods we actually
 *   need from @ref LocalStore.
 */
