...
void ChrootLinuxDerivationBuilder::deleteTmpDir(bool force)
{
...
bool ChrootLinuxDerivationBuilder::needsHashRewrite()
{
    return false;
}
...
void ChrootLinuxDerivationBuilder::setBuildTmpDir()
{
...
Path ChrootLinuxDerivationBuilder::tmpDirInSandbox()
{
...
void ChrootLinuxDerivationBuilder::prepareSandbox()
{
    ...
    if (buildUser
        && chown(chrootRootDir.c_str(), buildUser->getUIDCount() != 1 ? buildUser->getUID() : 0, buildUser->getGID())
               == -1)
...
Strings ChrootLinuxDerivationBuilder::getPreBuildHookArgs()
{
    assert(!chrootRootDir.empty());
    return Strings({store.printStorePath(drvPath), chrootRootDir});
}

Path ChrootLinuxDerivationBuilder::realPathInSandbox(const Path & p)
{
...
#  endif
    ...
    if (pathExists("/dev/pts/ptmx") && !pathExists(chrootRootDir + "/dev/ptmx") && !pathsInChroot.count("/dev/pts")) {
...
std::variant<std::pair<BuildResult::Status, Error>, SingleDrvOutputs> ChrootLinuxDerivationBuilder::unprepareBuild()
{
...
void ChrootLinuxDerivationBuilder::cleanupBuild()
{
...
void ChrootLinuxDerivationBuilder::addDependency(const StorePath & path)
{
    if (isAllowed(path))
        return;
