...
grep -q "NAR hash mismatch" <<< "$output" || fail "Expected 'NAR hash mismatch' in error output"
! grep -q "Assertion.*failed" <<< "$output" || fail "Should not contain assertion failure"
! grep -q "Aborted" <<< "$output" || fail "Should not contain 'Aborted'"